...
def main():
    ...
    while pipeline.running:
        ...
        # Make sure that the results are segmentation masks
        mask = results[0]
        if not isinstance(mask, xnornet.SegmentationMask):
            print("This sample requires a segmentation model to be installed. "
                  "Please install a segmentation model!", file=sys.stderr)
            pipeline.stop()
            break
