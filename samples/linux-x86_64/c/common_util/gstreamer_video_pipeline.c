...
static void on_bus_sync_message(GstBus* bus, GstMessage* message,
  ...
  if (strcmp(gst_structure_get_name(message_struct), "prepare-window-handle") ==
      0) {
    GstElement* imagesink = GST_ELEMENT(GST_MESSAGE_SRC(message));
    g_object_set(G_OBJECT(imagesink), "force-aspect-ratio", 1, NULL);
    GdkWindow* window = gtk_widget_get_window(GTK_WIDGET(pipeline->window));
    if (window == NULL) {
      errorf(pipeline, "Couldn't get GDK window");
      return;
    }
    gst_video_overlay_set_window_handle(
        GST_VIDEO_OVERLAY(imagesink),
...
static GstElement* make_video_source(xg_pipeline* pipeline) {
  GstElement* v4l2src = make_element(pipeline, "v4l2src", "source_v4l2src");
  GstElement* capsfilter =
      make_element(pipeline, "capsfilter", "source_capsfilter");
  GstElement* jpegdecoder = make_element(pipeline, "jpegdec", "source_jpegdec");
  ...
  GstCaps* caps_settings =
      gst_caps_from_string("image/jpeg,framerate=[10/1,30/1],width=[640,1280]");
...
static GstElement* make_auto_sink(xg_pipeline* pipeline) {
  ...
  sink = make_element(pipeline, "autovideosink", "auto_sink");
...
static void build_video_overlay_pipeline(xg_pipeline* pipeline) {
  ...
  source = make_video_source(pipeline);
...
static xg_pipeline* xg_create_base_pipeline(const char* window_title) {
  ...
  pipeline->window = GTK_WINDOW(gtk_window_new(GTK_WINDOW_TOPLEVEL));
  // Configure the window properties
  gtk_window_set_title(pipeline->window, window_title);
  gtk_window_set_default_size(pipeline->window, 1280, 960);
  // Register ourselves for keyboard events, so we can pause, stop, etc
  gtk_widget_add_events(GTK_WIDGET(pipeline->window), GDK_KEY_PRESS_MASK);
  if (g_signal_connect(pipeline->window, "key-press-event",
                       G_CALLBACK(on_key_press_event), pipeline) == 0) {
    errorf(pipeline, "Couldn't connect key-press signal to window");
    return NULL;
  }
  if (g_signal_connect(pipeline->window, "destroy",
                       G_CALLBACK(on_destroy_event), pipeline) == 0) {
    errorf(pipeline, "Couldn't connect destroy signal to window");
    return NULL;
  }
...
xg_pipeline* xg_create_video_overlay_pipeline(const char* window_title) {
  ...
  build_video_overlay_pipeline(pipeline);
  if (pipeline->error_occurred) {
