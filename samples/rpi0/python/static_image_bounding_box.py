...
def main(args=None):
    ...
    for box in boxes:
        if not isinstance(box, xnornet.BoundingBox):
            parser.error("The model you chose isn't an object detection "
                         "model, so we can't draw bounding boxes.  Please "
                         "use an object detection model for this demo.")
